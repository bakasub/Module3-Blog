...
const PostService = require('B:\\codegym\\casemodul3\\Module3-Blog\\service\\postService.js')
    ...
    static showHomeUser(req, res) {
        fs.readFile('./views/user.html', 'utf-8', async (err, createHtmlUser) => {
            if (err) {
                ...
                console.log(BlogRouting.userTotal.username)
    ...
    static profile(req,res,idUser){
        fs.readFile('./views/profile.html', 'utf-8', async (err, createHtml) => {
            if (err) {
                ...
                createHtml = createHtml.replace('{id}', idUser)
...
// quannnnnnnnn
    ...
    static DeleteBlog(req, res, id) {
        ...
        if (req.method === "GET") {
            fs.readFile('./views/admin/adminDeleteUser.html', 'utf-8', async (err, deleteHtml) => {
                if (err) {
                    console.log(err.message)
                } else {
